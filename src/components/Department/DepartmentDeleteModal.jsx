...
export default function DepartmentDeleteModal({
  ...
  const handleDelete = async () => {
    ...
    setError(null);
    try {
      const res = await deleteDepartmentApi(department.id);
      ...
      if (triggerToast) {
        triggerToast(`Department "${department.name}" deleted successfully!`);
      }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
      <div className="w-full max-w-md bg-white border border-slate-200 rounded-2xl p-6 text-slate-900 shadow-2xl animate-cardFadeUp">
        <div className="flex items-center gap-3 mb-4 pb-3 border-b border-slate-100">
          ...
          <div>
            <h3 className="text-base font-bold text-slate-900 m-0">Confirm Delete Department</h3>
            <p className="text-xs text-slate-500 m-0">
              Department: <strong className="text-slate-800">{department.name}</strong> (ID: #{department.id})
            </p>
          </div>
        </div>
